...
- 新增 `supplies` 和 `purchase_records` 两张表；持续采购记录关联物品，一次性采购记录保留自己的名称、规格、分类和单位。
- 采购数量使用物品的统一单位；已有采购记录后不能更改该物品的单位，避免改变历史数量的含义。
- 实付金额填写这条物品的总实付金额，不是单价；同一平台订单分多条录入时请分摊金额。
- 每条记录可选择一张不超过 2 MB 的 JPG、PNG 或 WebP 订单截图，浏览器会先压缩到不超过约 500 KB 再保存。截图保存在 D1，独立读取接口需要后台口令认证，列表不传输图片数据。
- 本地测试使用 Node.js 22.13+ 的内置 SQLite，不需要额外数据库依赖。

已有数据库应先执行采购迁移，再发布代码；新数据库直接使用完整 `schema.sql`：

```bash
npx wrangler d1 execute bakery-db --remote --file=./migrations/0003_procurement.sql
npm run deploy
```

本地已有数据库使用 `--local` 替换 `--remote`。采购迁移可重复执行，不更改已有商品和销售订单。

### 创始人决策记录

后台「决策」用于保存试验结论、最终决定、可复用原则和适用范围，与采购和财务数据分开。记录可以标记为有效、已替代或已归档。新数据库会自动加入“三轮车台面安装”这条初始记录。

已有数据库应先执行决策记录迁移，再发布代码；迁移可重复执行，初始记录只会加入一次：

```bash
npx wrangler d1 execute bakery-db --remote --file=./migrations/0006_founder_decisions.sql
npm run deploy
```
