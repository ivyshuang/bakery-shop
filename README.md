...
后台「采购」包含采购记录和物品清单。物品区分「一次性采购 / 持续采购」，可按类型筛选；采购支持新增、编辑、按物品搜索和查看历次购买。录入采购时可以直接新建物品。此版本不包含备注、库存位置、库存余额或消耗流水。

- 新增 `supplies` 和 `purchase_records` 两张表；每条采购记录关联一种物品。
- 采购数量使用物品的统一单位；已有采购记录后不能更改该物品的单位，避免改变历史数量的含义。
- 实付金额填写这条物品的总实付金额，不是单价；同一平台订单分多条录入时请分摊金额。
- 每条记录可保存一张不超过 500 KB 的 JPG、PNG 或 WebP 订单截图。截图保存在 D1，独立读取接口需要后台口令认证，列表不传输图片数据。
